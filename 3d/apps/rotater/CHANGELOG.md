...
- **Crop mode toggle behavior refined**: clicking a crop ratio button activates crop mode and updates the crop overlay to the selected ratio. If crop mode is already active and you click the same ratio again, it closes crop mode. Switching to a different ratio while already in crop mode just updates the overlay. Re-clicking the same ratio when crop is inactive re-opens crop mode while preserving the camera position (no reframe). Clicking outside the crop frame area (in the dimmed overlay) also closes crop mode. After page refresh, the toggle state is properly maintained.
