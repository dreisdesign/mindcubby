...
- **Improved Crop Mode usability**:
    - **Excluded viewport controls from auto-close**: Clicking D-pad arrows, Play/Pause, Inspect, or Theme/Settings buttons no longer cancels Crop Mode. These controls are now fully accessible while framing a crop.
    - **Updated Square ratio label**: Renamed `1:1` to `4:4` in the ratio selector.
...
### Fixed
- **Crop mode now preserves camera position on entry and re-entry**: clicking a crop ratio now enters crop mode without reframing the model, re-clicking the same ratio closes crop mode, and clicking the same ratio again re-opens crop mode without moving the model. Ratio changes while crop mode is active now update only the visible crop frame.
- **Crop mode outside-close is now reliable**: clicking anywhere in the dimmed crop area outside the frame closes crop mode, including empty space around the bottom D-pad region. The close check now uses the crop frame rect directly instead of relying on canvas-only hit testing.
- **Crop button hotspot and circle shape polish**: the crop expand/collapse button now keeps a true circular hit area, and the child icon/text no longer steals hover/cursor focus from the button surface.
...
- **Crop mode toggle behavior refined**: clicking a crop ratio button activates crop mode and updates the crop overlay to the selected ratio. Switching to a different ratio while already in crop mode just updates the overlay without closing. If crop mode is already active and you click the same ratio again, it closes crop mode. Re-clicking the same ratio when crop is inactive re-opens crop mode while preserving the camera position. Clicking outside the crop frame area (in the dimmed overlay) closes crop mode. Cursor hover over crop buttons now displays as pointer for the entire button area.
...
- **Crop cancel button now uses a reset icon and active purple color**: the crop control cancel button (which resets crop selection) now displays as a reset/refresh icon in the active purple color (`--palette-blueberry-500`) instead of a lighter purple, improving visual consistency with other active/selected controls in the app and better indicating its "reset" purpose.
