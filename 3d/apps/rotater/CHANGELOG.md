...
- **Crop mode toggle behavior refined**: clicking a crop ratio button activates crop mode and updates the crop overlay to the selected ratio. If crop mode is already active and you click the same ratio again, it closes crop mode. Switching to a different ratio while already in crop mode now just updates the overlay without closing. This provides an intuitive on/off toggle when re-clicking the same ratio, while still allowing ratio changes within active crop mode.
