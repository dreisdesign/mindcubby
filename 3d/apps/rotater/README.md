...
**Version (current workspace): 2.2.49**

Documentation sync: 2026-06-03
- Latest updates documented in `CHANGELOG.md` include neutral-color shade fixes for HSL/HSB (no red/pink tint on custom white), stable custom-preset switching, model-card shade/surface scope isolation, reliable single-select part activation when Multi-select is off, and improved export workspace click-close reliability.
...
- **About Rotater**: opens version/build info and project links, keeps Etsy first, and now uses the provided full-width logos for Etsy, Printables, YouTube, and GitHub in the links list. The panel shortcut list remains keyboard-only. Both the About header and the main app header prefer the full wordmark and only fall back to the compact `R` mark at tighter width thresholds. The canvas About button shows a NEW badge after an update until the panel is opened once.
