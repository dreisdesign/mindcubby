...
def get_root_dir():
    """Get SCRIPTS--STACKABLES root directory"""
...
DIR_ALIGNED = os.path.join(ROOT_DIR, "Output")  # Auto-detect Output_YYYYMMDD_HHMMSS
DIR_ZIPS = os.path.join(ROOT_DIR, "Output", "PACKAGED-ZIPS-CURRENT")
DIR_ARCHIVE = os.path.join(ROOT_DIR, "Output", "PACKAGED-ZIPS-ARCHIVE")
DESC_FILE = os.path.join(ROOT_DIR, "CATALOG", "Description.txt")

# GRID SPACING (for Blender alignment, if needed)
X_SPACING = 350.0  
Y_SPACING = 350.0  
...
def process_blender_logic():
    """Import, align, and export centered STLs into subfolders."""
    # Find latest Output folder or use selected
    output_parent = os.path.join(ROOT_DIR, "Output")
    if not os.path.exists(output_parent):
        print(f"ERROR: No Output folder found in {ROOT_DIR}")
        return 0
    ...
    if SELECTED_OUTPUT_FOLDER:
        latest_output = os.path.join(output_parent, SELECTED_OUTPUT_FOLDER)
        ...
        if outputs:
            latest = outputs[-1]
            latest_output = os.path.join(output_parent, latest)
            print(f"✓ Using latest Output folder: {latest}")
    
    if not latest_output or not os.path.exists(latest_output):
        print(f"ERROR: No Output folder found")
        return 0

    aligned_output = os.path.join(latest_output, "ALIGNED")
    
    if not os.path.exists(latest_output):
        print(f"ERROR: Output not found: {latest_output}")
        return 0

    # Ensure clean aligned output dir
    if os.path.exists(aligned_output):
        shutil.rmtree(aligned_output)
    os.makedirs(aligned_output)

    count = 0
    # Process all STL files in the output folder (recursive)
    for root, dirs, files in os.walk(latest_output):
        # Skip ALIGNED folder itself
        if "ALIGNED" in root:
            continue
    ...
    # Clear old STLs to prevent .001 duplicates
    print("Cleaning up old STL files...")
    for root, dirs, files in os.walk(aligned_output):
        for f in files:
            if f.endswith('.stl'):
                try:
    ...
    for obj in bpy.data.objects:
        if "--" in obj.name:
            ...
            out_dir = os.path.join(aligned_output, f_main)
...
def run_packaging_logic():
    ...
    # If SELECTED_OUTPUT_FOLDER is provided, use it
    if SELECTED_OUTPUT_FOLDER:
        output_folder_name = SELECTED_OUTPUT_FOLDER
        aligned_base = os.path.join(output_parent, SELECTED_OUTPUT_FOLDER, "ALIGNED")
        print(f"✓ Using selected Output folder: {SELECTED_OUTPUT_FOLDER}")
    else:
        # Otherwise, find latest (new format: Stackables_AUTO--...)
        outputs = sorted([
            d for d in os.listdir(output_parent)
            if (d.startswith('Stackables_AUTO--') or d.startswith('Output_')) and os.path.isdir(os.path.join(output_parent, d))
        ])
        
        if outputs:
            latest = outputs[-1]
            output_folder_name = latest
            aligned_base = os.path.join(output_parent, latest, "ALIGNED")
            print(f"✓ Using latest Output folder: {latest}")
    
    if not aligned_base or not os.path.exists(aligned_base):
        print(f"ERROR: No ALIGNED folder found")
        return
    
    # Set DIR_ZIPS and DIR_ARCHIVE to be inside this output folder
    DIR_ZIPS = os.path.join(output_parent, output_folder_name, "PACKAGED-ZIPS")
    DIR_ARCHIVE = os.path.join(output_parent, output_folder_name, "PACKAGED-ZIPS-ARCHIVE")
    
    print(f"Starting Etsy Zipping from {aligned_base}...")
    ...
    old_items = [f for f in os.listdir(DIR_ZIPS) if not f.startswith(".")] if os.path.exists(DIR_ZIPS) else []
    if old_items:
    ...
    variants = sorted([d for d in os.listdir(aligned_base) if os.path.isdir(os.path.join(aligned_base, d))])
    
    for variant in variants:
        variant_path = os.path.join(aligned_base, variant)
...
def generate_report(count):
    ...
    report_path = os.path.join(ROOT_DIR, "WORKFLOW_REPORT.txt")
    
    content = []
    content.append("=== STACKABLES WORKFLOW REPORT ===")
...
def main():
    print(f"\n--- STARTING STACKABLES PACKAGING WORKFLOW ---")
    setup_blender()
...
if __name__ == "__main__":
    ...
    # Check if we should signal completion (01D is the final step)
    if 'CONTINUE_WORKFLOW' in dir() and CONTINUE_WORKFLOW:
        print(f"\n{'='*60}")
        print(f"✓✓✓ WORKFLOW COMPLETE: 1A→1B→1C→1D ✓✓✓")
        print(f"{'='*60}\n")
